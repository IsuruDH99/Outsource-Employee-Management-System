...
const KPI = () => {
      ...
      <div className="w-full max-w-5xl bg-white p-6 rounded-lg shadow-lg">
        ...
        <div className="flex justify-center mb-4">
          <button
            ...
            className={`px-4 py-2 rounded-l ${activeTab === "Tab1" ? "bg-blue-500 text-white" : "bg-gray-300"}`}
          >
            Tab 1
          </button>
          <button
            onClick={() => setActiveTab("Tab2")}
            className={`px-4 py-2 rounded-r ${activeTab === "Tab2" ? "bg-blue-500 text-white" : "bg-gray-300"}`}
          >
            Tab 2
          </button>
        </div>
  ...
  <div className="p-6 w-full max-w-2xl mx-auto border rounded-lg shadow-lg">
            ...
            <select className="mb-4 w-full p-2 border rounded" value={product} onChange={(e) => setProduct(Number(e.target.value))}>
      ...
      <div className="space-y-2 p-2">
        {employeesList.map((employee) => (
          ...
          >
            ...
            <button onClick={calculateTargetTime} className="mb-1 w-full p-2 bg-blue-500 text-white rounded">
          ...
          <tbody>
            {[
              ...
              const handleSaveEmployee = () => {
                console.log(`✅ Saved KPI for ${employee.name} on ${selectedDate}`);
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const index = updated.findIndex(
                            (d) => d.epf === employee.epf && d.productCode === employeeData[0].code
                          );
                          if (index !== -1) {
                            updated[index].actualTime = parseFloat(e.target.value);
                            updated[index].kpi = (
                              updated[index].targetTime / updated[index].actualTime
                            ).toFixed(2);
                          } else {
                            updated.push({
                              epf: employee.epf,
                              ...
                              kpi: (
                                employeeData[0].targetTime / parseFloat(e.target.value)
                              ).toFixed(2)
                            });
                          }
                          setKpiData(updated);
                    ...
                    <td className="p-2 border">
                      <input
                        ...
                        onChange={(e) => {
                          const updated = [...kpiData];
                          const index = updated.findIndex(
                            (d) => d.epf === employee.epf && d.productCode === employeeData[1].code
                          );
                          if (index !== -1) {
                            updated[index].actualTime = parseFloat(e.target.value);
                            updated[index].kpi = (
                            ...
                            updated.push({
                              ...
                              actualTime: parseFloat(e.target.value),
                              kpi: (
                                employeeData[1].targetTime / parseFloat(e.target.value)
                              ).toFixed(2)
                            });
                          }
                          setKpiData(updated);
